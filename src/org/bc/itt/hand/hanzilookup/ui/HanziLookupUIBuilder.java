 ...
 */
package org.bc.itt.hand.hanzilookup.ui;
...
import org.bc.itt.hand.kiang.chinese.font.ChineseFontChooserFactory;
...
public class HanziLookupUIBuilder {
     ...
     * @return
     */
    static public JMenu buildOptionsMenu(HanziLookup lookup, Collection containers, ResourceBundle bundle) {
    ...
    static public JMenu buildCharacterModeMenu(final HanziLookup lookup, ResourceBundle bundle) {
        ...
        int searchType = lookup.getSearchType();
        
        // Need to declare as final so we can use them in the the anonymous listener definition.
        final JRadioButtonMenuItem simplifiedButton		= new JRadioButtonMenuItem(bundle.getString(HanziLookupBundleKeys.SIMPLIFIED_TYPE_BUNDLE_KEY), CharacterTypeRepository.SIMPLIFIED_TYPE == searchType);
        final JRadioButtonMenuItem traditionalButton	= new JRadioButtonMenuItem(bundle.getString(HanziLookupBundleKeys.TRADITIONAL_TYPE_BUNDLE_KEY), CharacterTypeRepository.TRADITIONAL_TYPE == searchType);
        final JRadioButtonMenuItem bothButton			= new JRadioButtonMenuItem(bundle.getString(HanziLookupBundleKeys.BOTH_TYPES_BUNDLE_KEY), CharacterTypeRepository.GENERIC_TYPE == searchType);
        
        // Anonymous listener handles the button clicking.
        ActionListener modeListener = new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                Object source = e.getSource();
                	
                if(source == simplifiedButton) {
                    lookup.setSearchType(CharacterTypeRepository.SIMPLIFIED_TYPE);
                } else if(source == traditionalButton) {
                    lookup.setSearchType(CharacterTypeRepository.TRADITIONAL_TYPE);
                } else {
                    lookup.setSearchType(CharacterTypeRepository.GENERIC_TYPE);
                }   
            }
        };
     ...
     * @return the menu item
     */
    static public JMenuItem buildFontMenuItem(final HanziLookup lookup, final Collection containers, ResourceBundle bundle) {
